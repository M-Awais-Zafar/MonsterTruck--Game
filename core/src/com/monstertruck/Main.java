...
public class Main extends ApplicationAdapter {
    ...
    int score = 0;
    int randGround = -1;
    int[] randPlatforms = new int[MAP_WIDTH];
    Vector2 point;
            ...
            public void run() {
                ...
                if (screen.equals("main")) { // MAIN
                    ...
                    btnSound.tex = new TextureRegion(assetManager.get(
                            pref.getBoolean("mute", false) ? "btnMute.png" : "btnSound.png", Texture.class));
                    ...
                    // generate level
                    if (randGround == -1) {
                        randGround = (int) Math.round(Math.random() * 25);
                        for (int i = 0; i < MAP_WIDTH; i++)
                            randPlatforms[i] = (int) Math.round(Math.random() * 40);
                    }

                    // ground
                    for (int i = 0; i < MAP_WIDTH; i++)
                        Lib.addLayer("g" + randGround, map, stage.getRoot(), mapPieceWidth * i);
                    ...
                    for (int j = 0; j < MAP_WIDTH; j++) {
                        Array<Act> actors = Lib.addLayer("p" + randPlatforms[j], map, stage.getRoot(), mapPieceWidth
                                * j);
    ...
    void showGroup(Group group) {
        ...
        for (int i = 0; i < str.length(); i++) {
            Act actor = new Act("", 0, 680, numbers.findRegion(str.substring(i, i + 1)));
            actors.add(actor);
    ...
    class CONTROL extends InputListener {
        ...
        public boolean touchDown(InputEvent event, float x, float y, int pointer, int button) {
            if (((Act) event.getTarget()).enabled) {
                ...
                if (screen.equals("game") && !isPaused && hero != null && hero.enabled) {
                    // controlLeft
        ...
        public boolean keyDown(InputEvent event, int keycode) {
            if (screen.equals("game") && hero != null && hero.enabled)
                switch (keycode) {
                    case Keys.LEFT:
                        keyLeft = true;
                        motorSound(true);
